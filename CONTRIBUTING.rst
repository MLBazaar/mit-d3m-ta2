...
mit-d3m-ta2 could always use more documentation, whether as part of the
official mit-d3m-ta2 docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `mit-d3m-ta2` for local development.

1. Fork the `mit-d3m-ta2` repo on GitHub.
...
2. The tests that cover a module called ``ta2/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/ta2/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_ta2
...
2. Update the version in ``setup.cfg``, ``ta2/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``ta2/__init__.py``
...
.. _GitHub issues page: https://github.com/HDI-Project/mit-d3m-ta2/issues
.. _Travis Build Status page: https://travis-ci.org/HDI-Project/mit-d3m-ta2/pull_requests
